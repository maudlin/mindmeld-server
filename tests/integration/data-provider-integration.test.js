...
describe('DataProvider Integration Tests', () => {
  ...
  describe('Provider Switching', () => {
    test('should switch between provider instances', async () => {
      const provider1 = await factory.createProvider({ type: PROVIDER_TYPES.LOCAL });
      
      // Create test data with first provider
      await provider1.save('switch-test-1', { n: [{ i: 'note1', c: 'First provider' }], c: [] });
  ...
  describe('Data Consistency', () => {
    test('should maintain data consistency across provider operations', async () => {
      ...
      const testData = {
        ...
        c: [
          { id: 'conn1', f: 'note1', t: 'note2', type: 'arrow' }
        ],
        meta: {
    ...
    test('should handle concurrent operations correctly', async () => {
      ...
      const maps = await provider.list();
      const concurrentMaps = maps.filter(map => map.id.startsWith('concurrent-'));
  ...
  describe('Error Handling and Edge Cases', () => {
    test('should handle invalid map data gracefully', async () => {
      ...
      await expect(provider.save('invalid-test', 'not an object')).rejects.toThrow();
    });
    ...
    test('should handle subscription errors gracefully', async () => {
      ...
      await expect(provider.save('error-test', { n: [], c: [] })).resolves.toBeDefined();
    });
    
    test('should handle missing maps correctly', async () => {
      const provider = await factory.createProvider();
      
      await expect(provider.load('does-not-exist')).rejects.toThrow('Map not found');
