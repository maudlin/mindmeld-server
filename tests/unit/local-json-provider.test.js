...
describe('LocalJSONProvider', () => {
  ...
  describe('Constructor and Initialization', () => {
    ...
    test('should test localStorage availability on initialization', () => {
      expect(localStorage.setItem).toHaveBeenCalledWith('test_map_test', 'test');
    ...
    test('should throw error if localStorage is not available', () => {
      ...
      expect(() => new LocalJSONProvider()).toThrow('LocalStorage not available');
    });
  });
  ...
  describe('DataProvider Contract - load()', () => {
    test('should load existing map successfully', async () => {
      ...
      const testMeta = { version: 1, title: 'Test Map', created: '2024-01-01T00:00:00.000Z' };
    ...
    test('should throw error for non-existent map', async () => {
      await expect(provider.load('non-existent-map')).rejects.toThrow('Map not found: non-existent-map');
    });
    
    test('should throw error for invalid mapId', async () => {
      await expect(provider.load('')).rejects.toThrow('Invalid mapId: must be a non-empty string');
      await expect(provider.load(null)).rejects.toThrow('Invalid mapId: must be a non-empty string');
    });
    ...
    test('should handle corrupted map data gracefully', async () => {
      ...
      await expect(provider.load(mapId)).rejects.toThrow(`Failed to load map ${mapId}`);
    });
  ...
  describe('DataProvider Contract - save()', () => {
    test('should save map data successfully', async () => {
      ...
      const savedMeta = JSON.parse(localStorage.setItem.mock.calls.find(
        call => call[0] === metaKey
      )[1]);
    ...
    test('should allow forced save when autosave is paused', async () => {
      ...
      const result = await provider.save(mapId, testData, { autosave: true, force: true });
    ...
    test('should validate map data before saving', async () => {
      ...
      await expect(provider.save(mapId, 'not an object')).rejects.toThrow('Invalid data');
    });
  });
  ...
  describe('DataProvider Contract - list()', () => {
    beforeEach(async () => {
      // Set up test data
      const testMaps = [
        { id: 'map1', title: 'First Map', created: '2024-01-01T00:00:00.000Z', modified: '2024-01-01T01:00:00.000Z' },
        { id: 'map2', title: 'Second Map', created: '2024-01-02T00:00:00.000Z', modified: '2024-01-02T01:00:00.000Z' },
        { id: 'map3', title: 'Third Map', created: '2024-01-03T00:00:00.000Z', modified: '2024-01-03T01:00:00.000Z' }
      ];
  ...
  describe('DataProvider Contract - delete()', () => {
    test('should delete existing map successfully', async () => {
      ...
      expect(localStorage.removeItem).toHaveBeenCalledWith('test_meta_' + mapId);
    });
  ...
  describe('DataProvider Contract - subscribe/unsubscribe()', () => {
    ...
    test('should require valid callback for subscription', async () => {
      await expect(provider.subscribe('test-map', null)).rejects.toThrow('Callback must be a function');
      await expect(provider.subscribe('test-map', 'not a function')).rejects.toThrow('Callback must be a function');
    });
  ...
  describe('LocalJSONProvider Specific Features', () => {
    test('should calculate storage statistics', () => {
      ...
      localStorage.setItem('test_meta_stat1', JSON.stringify({ title: 'Test' }));
    ...
    test('should reject invalid import data', async () => {
      await expect(provider.importMaps({})).rejects.toThrow('Invalid export data structure');
      await expect(provider.importMaps({ maps: null })).rejects.toThrow('Invalid export data structure');
    });
  });
  ...
  describe('Storage Management', () => {
    test('should clean up storage when too many maps exist', async () => {
      ...
      for (let i = 1; i <= 15; i++) {
        ...
        localStorage.setItem(`test_map_${mapId}`, JSON.stringify({ n: [], c: [] }));
      }
    ...
    test('should remove invalid metadata entries during cleanup', () => {
      ...
      for (let i = 1; i <= 12; i++) {
        localStorage.setItem(`test_meta_cleanup${i}`, JSON.stringify({ modified: `2024-01-${i.toString().padStart(2, '0')}T00:00:00.000Z` }));
      ...
      expect(localStorage.removeItem).toHaveBeenCalledWith('test_meta_invalid1');
  ...
  describe('Error Handling', () => {
    test('should handle localStorage errors gracefully', async () => {
      ...
      await expect(provider.load('error-test')).rejects.toThrow('Failed to load map error-test');
    });
